...
<head>
	...
	<script type="text/javascript">
		...
		function insertView(){
			location.href = "/qsolcowork/Manager/Insertform";
		}
...
<!-- <body style="background-color:#d4d4d4"> -->
	...
	<div class="viewListTop">
	    ...
	    <form class="navbar-form navbar-right" style="border-right-width: 2px; border-right-style: solid; border-right-color: gray; margin-right: 5px;">
			<select class="btn btn-primary" name="SearchType">
	            <option value="managername"<c:if test="${pagingVO.searchType == 'managername'}">seleted</c:if> >담당자명</option>
	            <option value="managerid" <c:if test="${pagingVO.searchType == 'managerid'}">seleted</c:if> >담당자ID</option>
	        </select>
	...
	<div class="viewList">
		...
		<table id="myTable" class="table table-striped tablesorter" >
			...
			<tbody align="center">
				<c:forEach items="${managerVO}" var="managerVO" varStatus="rowCount">
					<tr>
						<td>${managerVO.managerid}</td>
						<td style="font-weight: bold;"><a href="${pageContext.request.contextPath}/Manager/View?managerid=${managerVO.managerid}">${managerVO.managername}</a></td>
