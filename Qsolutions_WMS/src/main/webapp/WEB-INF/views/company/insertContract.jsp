...
<form action="${pageContext.request.contextPath}/Company/Insert" method="post" id="insertData" name="insertData">
    <div class="viewListTop">
    	<span class="sub-header" style="margin-left: 10px; position: relative; font-size: 30px; font-weight: bold;">계약 등록</span>
    	<button type="button" class="btn btn-primary pull-right" onclick="insertdata()" style="margin-right: 10px; margin-top: 8px;">계약 등록</button>
    </div>
		...
		<p class="sub-header" style="margin-left: 10px; margin-top:20px; font-size: 15px; font-weight: bold;">고객사 명</p>
			<select class="selectpicker show-tick" data-style="btn-primary" name="companycode" id="companycode" data-live-search="true" data-width="100%" data-size="10" title="== 고객사를 선택해주세요 ==" style="display: inline-block;">
				<c:forEach var="companyList" items="${companyList}" varStatus="list">
					<option id="companycode" value="${companyList.companycode}"<c:if test="${companyList.companycode == tmpcode}">selected="selected"</c:if>>${companyList.companyname}</option>
				</c:forEach>
			</select>
			...
			<select class="form-control" id="contractsubject" name="contractsubject">
				<option value="" selected disabled hidden>== 계약 분류를 선택해주세요 ==</option>
				<option value="유지보수">유지보수</option>
				<option value="판매">판매</option>
				<option value="기타">기타</option>
			</select>
		<p class="sub-header" style="margin-left: 10px; margin-top:20px; font-size: 15px; font-weight: bold;">계약명</p>
		<input id="contracttitle" name="contracttitle" type="text" class="form-control" placeholder="" size="50" style="width: 100%; display: inline-block; ">
		
		<p class="sub-header" style="margin-left: 10px; margin-top:20px; font-size: 15px; font-weight: bold;">계약 내용</p>
			<textarea id="contractname" name="contractname" class="form-control summernote hint2emoji" rows="3" placeholder="업무 내용을 입력해주세요.." style="width: 100%; display: inline-block;" required></textarea>
		...
		<p class="sub-header" style="margin-left: 10px; margin-top:20px; font-size: 15px; font-weight: bold;">계약 기간</p>
			...
			<div class="demo" style="margin-bottom: 50px;">
				<p id="datepairExample">
					<input type="text" class="form-control date start" id="startdate" name="startdate" placeholder="시작 날짜.." size="50" style="width: 48%; display: inline-block;" autocomplete="off" /> to
					<input type="text" class="form-control date end pull-right" id="enddate" name="enddate" placeholder="종료 날짜.." size="50" style="width: 48%; display: inline-block;" autocomplete="off" />
				</p>
			</div>
